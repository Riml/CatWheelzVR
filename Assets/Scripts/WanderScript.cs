...
public class WanderScript : MonoBehaviour {
	...
	public float maxDistance = 3; //how far away the animal can walk from its starting position | True maxRange is maxDistance + 1 (A cousion is added)

	public float minIdle = 1; //the minimum time length the animal will stand idle before walking again | must be 0 or greater
	public float maxIdle = 10; //the maximum time length the animal will stand idle before walking again | must be greater than the min

	public float minWalk = 2; //the minimum time length the animal will walk before standing still again | must be 0 or greater
	public float maxWalk = 5; //the maximum time length the animal will walk before standing still again | must be greater than the min
	...
	void  Start (){
		...
		float randomRot= Random.Range(0f, 360f); //get a random rotation
	...
	void  NewDecision (){
		...
		//else if idle, start walking
			...
			//else, face startPos, and walk back
				...
				//face startPos
				Quaternion neededRotation= Quaternion.LookRotation(startPos - transform.position);
